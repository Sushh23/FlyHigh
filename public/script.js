...
const showResults = (results) => {
  ...
  results.forEach(({ itineraries, price }) => {
    const priceLabel = `${price.total}`;
    var priceLabelR = `${Math.round(priceLabel*88.2)}`;
    var priceLabelINR = priceLabelR + " INR";
    ...
    searchResults.insertAdjacentHTML(
      ...
      `<li class="flex-column flex-sm-row list-group-item d-flex justify-content-between align-items-sm-center">
        ...
        <button class="bg-primary rounded-pill m-2 badge fs-6" type="button" onclick="window.location.href='displayseats.html';" style = "background-color : #140038;">${priceLabelINR}</button>
      </li>` 
    );
  });
};
searchButton.addEventListener("click",()=>{
  ...
  const infant = infantsInput.value;
  localStorage.setItem("infant",infant);
});
document.addEventListener("click", (event) => {
  const target = event.target;
  if (target.classList.contains("bg-primary") && target.classList.contains("rounded-pill") && target.classList.contains("badge") && target.classList.contains("fs-6")) 
  {
    const cost = target.textContent.trim();
    setCost(cost);
  }
});
const setCost = (cost)=>{
